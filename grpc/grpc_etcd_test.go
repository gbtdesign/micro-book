...
import (
	"context"
	_ "golang/webook/pkg/grpcx/balancer/wrr"
...
func (s *EtcdTestSuite) TestCustomRoundRobinClient() {
	// 在测试函数内
	b := balancer.Get("custom_wrr")
	s.T().Logf("custom_wrr balancer: %v", b)
	if b == nil {
		s.T().Fatalf("custom_wrr not registered")
	}
...
`
	...
	s.T().Logf("cc: %p, err: %v", cc, err)
	require.NoError(s.T(), err)
	if cc == nil {
		s.T().Fatalf("cc is nil")
	}
